...
func scrapeViaFleet(ctx context.Context, pageURL string) (markdown string, renderedHTML string, final *url.URL, err error) {
	...
	markdown = ws.GetContent()
	renderedHTML = ws.GetRenderedHtml()
	finalU, err := url.Parse(ws.GetSourceUrl())
	if err != nil {
		return "", "", nil, err
	}
	slog.Info("p_seer_websites: fleet scrape returned page",
		"url", pageURL,
		"final_url", finalU.String(),
		"markdown_bytes", len(markdown),
		"html_bytes", len(renderedHTML),
	)
	if strings.TrimSpace(markdown) == "" {
		return "", "", nil, fmt.Errorf("fleet scrape returned empty content for %s (check nodescraper logs; Servo runtime deps must be available on the node)", pageURL)
	}
	return markdown, renderedHTML, finalU, nil
}

// ScrapeToMarkdown validates URL, fetches rendered page via the node fleet, returns markdown and canonical URL.
func ScrapeToMarkdown(ctx context.Context, rawURL string) (markdown string, canonical *url.URL, err error) {
	...
	md, _, finalU, err := scrapeViaFleet(ctx, canon.String())
	if err != nil {
	...
	if strings.TrimSpace(md) == "" {
		return "", nil, fmt.Errorf("no extractable text from page (fleet returned empty markdown from %s)", finalU.String())
	}
	out := cloneURL(canon)
	if finalU != nil {
